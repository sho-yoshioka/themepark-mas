...
public class ThemeParkGraph extends Graph{
	 ...
	 * @param n グラフの頂点の個数
	 */
	public static void dijkstra(Graph graph, int source, int n) {
		...
		//各頂点'i'までの経路をprev[]を使用して求める
		for (int i = 0; i < n; i++) {
			//始点以外で最短経路が更新されているノード'i'に対して
...
class Main {
	public static void main(String[] args) {
		...
		for(int source = 0; source < ThemeParkGraph.GRAPH_SIZE; source++) {
			ThemeParkGraph.dijkstra(themeParkGraph, source, ThemeParkGraph.GRAPH_SIZE);
		}
	}
}
